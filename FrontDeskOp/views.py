...
def login_FrontDeskOp(request):    
    if request.method == "POST":
        m = mysql.connector.connect(host="localhost",user="root",passwd="root",database='HMS')
        print("HI3")
        cursor = m.cursor()
        d = request.POST
...
def patient_reg(request):
   if request.method == "POST":
    ...
    cnx = mysql.connector.connect(host="localhost",user="root",passwd="root",database='HMS')
    cursor = cnx.cursor()
    d = request.POST
